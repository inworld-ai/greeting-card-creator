...
${allQuestionsAnswered ? `🚨🚨🚨 FINAL WARNING: ALL QUESTIONS ARE ANSWERED! 🚨🚨🚨\n\nIf ALL questions have been answered (check the list above), you MUST:\n1. Give a BRIEF, warm reaction to their answer (1-2 sentences max)\n2. Then IMMEDIATELY say: "All set! I'll create your Christmas card for ${recipientName}."\n\nABSOLUTELY FORBIDDEN:\n- DO NOT ask "what's a funny or heartwarming anecdote" or ANY question\n- DO NOT ask about anything else\n- DO NOT repeat any questions\n- DO NOT ask follow-up questions\n- DO NOT say "So, what's..." or "I'd love to hear..."\n\nYour response when all questions are answered MUST end with: "All set! I'll create your Christmas card for ${recipientName}."\n\nIf you ask ANY question when all questions are answered, you have FAILED. The conversation is OVER. Say the wrap-up message NOW.` : nextQuestion && nextQuestion.key === 'funnyStory' ? `🚨🚨🚨🚨🚨 FINAL WARNING: YOU MUST ASK THE ANECDOTE QUESTION! 🚨🚨🚨🚨🚨\n\nThis is the SECOND and FINAL question. You MUST ask it NOW.\n\nIf there is a NEXT question to ask ("${nextQuestion.question}"), you MUST:\n1. Give a BRIEF, warm reaction to their answer about ${recipientName} (1-2 sentences max, like "Oh, that's so sweet!" or "That's wonderful!")\n2. Then IMMEDIATELY ask: "${nextQuestion.question}"\n\nDO NOT just comment on their answer and stop.\nDO NOT wait for them to say something else.\nDO NOT skip asking this question.\nDO NOT end your response without asking the anecdote question.\n\nYour response MUST include and END WITH the question: "${nextQuestion.question}"\n\nExample: "Oh, that's so sweet! Missy sounds wonderful. 😊 What's a funny or heartwarming anecdote about Missy?"\n\nAfter asking this question, wait for their answer.` : nextQuestion ? `If there is a NEXT question to ask ("${nextQuestion.question}"), you MUST:
1. Give a BRIEF, warm reaction to their answer (1-2 sentences max, like "Oh, that's lovely!" or "That's wonderful!")
2. Then IMMEDIATELY ask: "${nextQuestion.question}"
