...
export class AssemblyAISTTWebSocketNode extends CustomNode {
    ...
    const messageHandler = (data: WebSocket.Data) => {
      try {
        ...
        if (msgType === 'Turn') {
          ...
          // Send FINAL transcript to client as well (not just partials)
          this.sendFinalTranscript(sessionId, nextInteractionId, transcript);
  ...
  private sendFinalTranscript(
    ...
    if (!connection || !connection.ws) {
      console.warn(
        `[AssemblyAI WS STT] Cannot send final transcript - no connection for session: ${sessionId}`,
      );
      return;
    }
    ...
    try {
      ...
      // Mark as final
      textEvent.text.final = true;
      connection.ws.send(JSON.stringify(textEvent));
      console.log(`[AssemblyAI WS STT] Sent final transcript to client: "${text}"`);
    } catch (error) {
      console.error(
        '[AssemblyAI WS STT] Error sending final transcript:',
        error,
      );
    }
  }
